...
pro.start = function() {
  ...
  /*
  this.acceptor.on('connection', function(obj) {
    self.emit('connection', obj);
  });
  */
  this.acceptor.on('connection', self.emit.bind(self, 'connection'));
  this.acceptor.listen(this.port);
};

pro.kickById = function(id, reason) {
  if(!this.started || this.stoped) {
    return;
  }
  this.acceptor.kickById(id, reason);
};
